namespace Gley.MobileAds.Internal
{
    public class Constants
    {
        internal const string GDPR_KEY = "UserConsent";
        internal const string CCPA_KEY = "CcpaConsent";
        internal const string REMOVE_ADS_KEY = "RemoveAds";
        internal const string SUBScription_KEY = "IsSubscriptionValid";
        public const string DATA_NAME_RUNTIME = "MobileAdsDataRuntime";
        public const string DATA_NAME_EDITOR = "MobileAdsDataEditor";
        public const string DATA_PATH_EDITOR = "Scripts/Editor/EditorSave";
        public const string RESOURCES_FOLDER = "Resources";
    }
}
...
namespace Gley.MobileAds.Internal
{
    public class MobileAdsManager : MonoBehaviour
    {
        ...
        public void SubscritionActive(bool active)
        {
            if (active == true)
            {
                PlayerPrefs.SetInt(Constants.SUBScription_KEY, 1);
                //if banner is active and user bought remove ads the banner will automatically hide
                HideBanner();
            }
            else
            {
                PlayerPrefs.SetInt(Constants.SUBScription_KEY, 0);
            }
        }
        /// <summary>
        ...
        {
            if (!PlayerPrefs.HasKey(Constants.REMOVE_ADS_KEY) && !PlayerPrefs.HasKey(Constants.SUBScription_KEY))
            {
                return true;
            }
            else
            {
                if (PlayerPrefs.GetInt(Constants.REMOVE_ADS_KEY) == 1 || PlayerPrefs.GetInt(Constants.SUBScription_KEY) == 1)
                {
                    return false;
                }
        ...
        public bool isRemoveAds()
        {
            ...
            {
                if (PlayerPrefs.GetInt(Constants.REMOVE_ADS_KEY) == 1)
                {
                    return true;
                }
        ...
        {
            if (!PlayerPrefs.HasKey(Constants.SUBScription_KEY))
            {
                return false;
            }
            else
            {
                if (PlayerPrefs.GetInt(Constants.SUBScription_KEY) == 1)
                {
                    return true;
                }
